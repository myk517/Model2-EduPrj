...
function Slider() {
  ...
  const goLeft = () => {
    buttonState === 0 ? buttonSetState(LAST_SLIDE_X_COORDINATE) : buttonSetState(buttonState + 100);
  };

  const goRight = () => {
    buttonState === LAST_SLIDE_X_COORDINATE ? buttonSetState(0) : buttonSetState(buttonState - 100);
  };
    ...
    <MainSlideContainer>
      <SlideContainerLeft className="SlideContainerLeft">
        {slideImageState.map((item) => {
          return (
            <Slide key={item.id} style={{ transform: `translateX(${buttonState}%)` }}>
      ...
      >
        {slideImageState.map((item) => {
          return (
            ...
            >
              <TextBoxComp clnassName="root" MainTitle={item.content} backgroundColor={item.color} SubTitle={item.subContent} />
            </SlideTextBox>
          );
        })}
